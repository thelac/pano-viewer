...
FV.onPanoError = function() {
  var bClick = false;
  var ath = FV.stView === undefined ? 0 : FV.stView.poi[0].ath;
  var side;
  var hash = FV.stView === undefined ? 0 : FV.stView.poi[0].pano; 
  ...
  var previewUrl;
  if(FV.stView === undefined)
    previewUrl = config.absModelURL + FV.panoURL + '1/1.jpg'; 
  else
  previewUrl = config.absModelURL + FV.stViewImgURL + hash + '/img_' + side + '.jpg';
  ...
  FV.utils.addClickEvent(flashFallback, function(e) {
    if(!bClick){
