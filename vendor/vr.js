...
define(function() {
  ...
  vr.PluginDataSource.prototype.load = function(callback, opt_scope) {
    ...
    // Wait until the plugin adds itself to the window.
    var startTime = Date.now();
    var self = this;

    function checkLoaded() {
      if (window._vr_native_) {
        self.native_ = window._vr_native_;
        callback.call(opt_scope, null);
      } else {
  ...
  vr.Runtime = function(document) {
    ...
    switch (this.dataSourceMode_) {
      ...
      case vr.DataSourceMode.DRIVER:
        dataSource = new vr.DriverDataSource(window['__vr_driver__']);
        break;
    }
   ...
   * @private
   */
  vr.runtime_ = new vr.Runtime(window.document);
  ...
  vr.enterFullScreen = function() {
    ...
    if (hmdInfo) {
      window.moveTo(hmdInfo.desktopX, hmdInfo.desktopY);
      window.resizeTo(hmdInfo.resolutionHorz, hmdInfo.resolutionVert);
    }
